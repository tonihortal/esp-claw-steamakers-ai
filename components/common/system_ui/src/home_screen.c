...
#define SYSTEM_UI_HOME_TITLE_Y_PCT 25
#define SYSTEM_UI_HOME_SUBTITLE_Y_PCT 34
#define SYSTEM_UI_HOME_CLOCK_CENTER_PCT 62
#define SYSTEM_UI_HOME_DATE_Y_PCT 76
...
static system_ui_home_layout_t system_ui_home_layout(void)
{
    ...
    int32_t subtitle_y = system_ui_clamp_i32(height * SYSTEM_UI_HOME_SUBTITLE_Y_PCT / 100,
                                             title_y + 20, height - 125);
    int32_t clock_h = s_ui.status_only
                          ? system_ui_clamp_i32(height * 12 / 100, 26, 42)
                          : system_ui_clamp_i32(height * 22 / 100, 44, 84);
    int32_t clock_w = system_ui_clamp_i32(width - system_ui_clamp_i32(short_side / 8, 28, 64), 176, 380);
    int32_t clock_center_y = system_ui_clamp_i32(height * SYSTEM_UI_HOME_CLOCK_CENTER_PCT / 100,
                                                 subtitle_y + 48, height - 60);
...
static void system_ui_home_update_clock_locked(void)
{
    ...
    if (now > 0 && localtime_r(&now, &tm_now)) {
        ...
        const char *weekday = tm_now.tm_wday >= 0 && tm_now.tm_wday < 7
                                  ? weekday_names[tm_now.tm_wday]
                                  : "---";
        snprintf(date_text, sizeof(date_text), "%s, %02d/%02d/%04d",
                 weekday, tm_now.tm_mday, tm_now.tm_mon + 1, tm_now.tm_year + 1900);
    } else {
...
static esp_err_t system_ui_home_create_identity_locked(const system_ui_home_layout_t *layout)
{
    int32_t text_w = system_ui_max_i32(80, (int32_t)s_ui.width - layout->pad * 2);

    if (s_ui.home_title[0]) {
        s_ui.title_label = lv_label_create(s_ui.home_tile);
        ESP_RETURN_ON_FALSE(s_ui.title_label != NULL, ESP_ERR_NO_MEM, SYSTEM_UI_TAG, "create title failed");
        lv_obj_set_width(s_ui.title_label, text_w);
        lv_label_set_long_mode(s_ui.title_label, LV_LABEL_LONG_CLIP);
        lv_obj_set_style_text_color(s_ui.title_label, system_ui_color(SYSTEM_UI_COLOR_TEXT), 0);
        lv_obj_set_style_text_align(s_ui.title_label, LV_TEXT_ALIGN_CENTER, 0);
        system_ui_apply_font(s_ui.title_label);
        lv_label_set_text(s_ui.title_label, s_ui.home_title);
        lv_obj_align(s_ui.title_label, LV_ALIGN_TOP_MID, 0, layout->title_y);
    }

    if (s_ui.home_subtitle[0]) {
        s_ui.subtitle_label = lv_label_create(s_ui.home_tile);
        ESP_RETURN_ON_FALSE(s_ui.subtitle_label != NULL, ESP_ERR_NO_MEM, SYSTEM_UI_TAG, "create subtitle failed");
        lv_obj_set_width(s_ui.subtitle_label, text_w);
        lv_label_set_long_mode(s_ui.subtitle_label, LV_LABEL_LONG_CLIP);
        lv_obj_set_style_text_color(s_ui.subtitle_label, system_ui_color(SYSTEM_UI_COLOR_MUTED), 0);
        lv_obj_set_style_text_align(s_ui.subtitle_label, LV_TEXT_ALIGN_CENTER, 0);
        system_ui_apply_font(s_ui.subtitle_label);
        lv_label_set_text(s_ui.subtitle_label, s_ui.home_subtitle);
        lv_obj_align(s_ui.subtitle_label, LV_ALIGN_TOP_MID, 0, layout->subtitle_y);
    }
...
static esp_err_t system_ui_create_home_tile_locked(void)
{
    ...
    ESP_RETURN_ON_ERROR(system_ui_home_create_identity_locked(&layout), SYSTEM_UI_TAG, "create identity failed");
