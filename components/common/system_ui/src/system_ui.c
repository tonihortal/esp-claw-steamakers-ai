...
#define SYSTEM_UI_STATUS_EVENT_TASK_STACK 3072
#define SYSTEM_UI_STATUS_EVENT_QUEUE_LEN 2
...
static esp_err_t system_ui_start_event_task(void)
{
    if (s_ui.event_queue == NULL) {
        s_ui.event_queue = xQueueCreate(s_ui.status_only ? SYSTEM_UI_STATUS_EVENT_QUEUE_LEN : SYSTEM_UI_EVENT_QUEUE_LEN,
    ...
    BaseType_t ok = xTaskCreate(system_ui_event_task,
                                "ui_event",
                                s_ui.status_only ? SYSTEM_UI_STATUS_EVENT_TASK_STACK : SYSTEM_UI_EVENT_TASK_STACK,
                                NULL,
...
esp_err_t system_ui_start(const system_ui_config_t *config)
{
    ...
    s_ui.generation++;
    s_ui.status_only = config && config->status_only;
    strlcpy(s_ui.home_title,
            config && config->home_title ? config->home_title : "",
            sizeof(s_ui.home_title));
    strlcpy(s_ui.home_subtitle,
            config && config->home_subtitle ? config->home_subtitle : "",
            sizeof(s_ui.home_subtitle));
    if (!s_ui.status_only) {
    ...
    if (ret == ESP_OK && !s_ui.status_only) {
