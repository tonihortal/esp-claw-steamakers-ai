...
#if CONFIG_ESP_BOARD_ESP32_STEAMAKERS_AI
#define APP_BOARD_SYSTEM_PROMPT \
    "On the esp32_steamakers_ai board, handle routine generic GPIO requests directly with " \
    "lua_run_script at /system/skills/steamakers_gpio/scripts/gpio_control.lua; do not activate " \
    "a GPIO or board-information skill first. Pass timeout_ms=10000. For digital output, args " \
    ...
    "steamakers_hardware_test and run every prescribed script sequentially in the same request; " \
    "never pause for another confirmation between scripts. " \
    "Never claim a physical result until the user confirms it.\n"
#else
#define APP_BOARD_SYSTEM_PROMPT ""
#endif
...
#if CONFIG_APP_CLAW_SYSTEM_UI_ENABLE && CONFIG_APP_CLAW_CAP_LUA && !CONFIG_APP_CLAW_SYSTEM_UI_STATUS_ONLY
...
{
#if CONFIG_APP_CLAW_SYSTEM_UI_ENABLE
    const system_ui_config_t ui_config = {
        .buffer_lines = CONFIG_APP_CLAW_SYSTEM_UI_BUFFER_LINES,
        .home_title = CONFIG_APP_CLAW_SYSTEM_UI_HOME_TITLE,
        .home_subtitle = CONFIG_APP_CLAW_SYSTEM_UI_HOME_SUBTITLE,
        .status_only = CONFIG_APP_CLAW_SYSTEM_UI_STATUS_ONLY,
    };
    ESP_RETURN_ON_ERROR(system_ui_start(&ui_config), TAG, "start system UI failed");
#if CONFIG_APP_CLAW_CAP_LUA && !CONFIG_APP_CLAW_SYSTEM_UI_STATUS_ONLY
                                      ...
                                      claw_core_config_t *core_config)
{
    ...
    core_config->task_stack_size = CONFIG_APP_CLAW_CORE_TASK_STACK_SIZE;
...
esp_err_t app_claw_start(const app_claw_config_t *config)
{
    ...
    claw_event_router_config_t router_config = {
        .rules_path = NULL,
        .task_stack_size = CONFIG_APP_CLAW_EVENT_ROUTER_TASK_STACK_SIZE,
        .task_priority = 5,
    ...
    ESP_RETURN_ON_ERROR(cap_scheduler_init(&(cap_scheduler_config_t) {
                            ...
                            .tick_ms = 1000,
                            .max_items = CONFIG_APP_CLAW_SCHEDULER_MAX_ITEMS,
                            .task_stack_size = CONFIG_APP_CLAW_SCHEDULER_TASK_STACK_SIZE,
                            .task_priority = 5,
                            .task_core = tskNO_AFFINITY,
...
#if CONFIG_APP_CLAW_SYSTEM_UI_ENABLE && !CONFIG_APP_CLAW_SYSTEM_UI_STATUS_ONLY
    ...
    /*
     * Capability gateways may create long-lived network tasks. Start the core,
     * router, scheduler, and temporary time-sync worker first so their stacks
     * can obtain contiguous internal RAM on boards without usable PSRAM.
     */
    ESP_RETURN_ON_ERROR(app_capabilities_start(), TAG, "Failed to start capabilities");
