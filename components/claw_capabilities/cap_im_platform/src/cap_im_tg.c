...
#define CAP_IM_TG_START_RETRY_DELAY_MS 5000
...
typedef struct {
    ...
    esp_timer_handle_t start_retry_timer;
...
static void cap_im_tg_start_retry_cb(void *arg)
{
    (void)arg;

    if (!s_tg.stop_requested && !s_tg.poll_task) {
        ESP_LOGI(TAG, "Retrying Telegram task startup");
        (void)cap_im_tg_gateway_start();
    }
}

static esp_err_t cap_im_tg_ensure_start_retry_timer(void)
{
    esp_err_t err;

    if (!s_tg.start_retry_timer) {
        const esp_timer_create_args_t timer_args = {
            .callback = cap_im_tg_start_retry_cb,
            .name = "tg_start_retry",
        };

        err = esp_timer_create(&timer_args, &s_tg.start_retry_timer);
        if (err != ESP_OK) {
            return err;
        }
    }
...
static esp_err_t cap_im_tg_schedule_start_retry(void)
{
    esp_err_t err = cap_im_tg_ensure_start_retry_timer();

    if (err != ESP_OK) {
        return err;
    }
    if (esp_timer_is_active(s_tg.start_retry_timer)) {
        return ESP_OK;
    }

    ESP_LOGW(TAG, "Telegram task memory unavailable; retrying startup in %d ms",
             CAP_IM_TG_START_RETRY_DELAY_MS);
    return esp_timer_start_once(s_tg.start_retry_timer,
                                CAP_IM_TG_START_RETRY_DELAY_MS * 1000ULL);
}
...
static esp_err_t cap_im_tg_gateway_start(void)
{
    ...
    ok = claw_task_create(&(claw_task_config_t){
                              .name = "tg_poll",
                              .stack_size = CONFIG_APP_CLAW_IM_TG_POLL_TASK_STACK_SIZE,
                              .priority = 5,
                              .core_id = tskNO_AFFINITY,
                              .stack_policy = CLAW_TASK_STACK_PREFER_PSRAM,
                          },
    ...
    if (ok != pdPASS) {
        ...
        return cap_im_tg_schedule_start_retry();
    }

    if (s_tg.start_retry_timer && esp_timer_is_active(s_tg.start_retry_timer)) {
        (void)esp_timer_stop(s_tg.start_retry_timer);
    }
...
static esp_err_t cap_im_tg_gateway_stop(void)
{
    ...
    if (s_tg.start_retry_timer) {
        ...
        (void)esp_timer_delete(s_tg.start_retry_timer);
        s_tg.start_retry_timer = NULL;
    }
    if (!s_tg.poll_task) {
        return ESP_OK;
    }
...
esp_err_t cap_im_tg_register_group(void)
{
    ...
    ESP_RETURN_ON_ERROR(cap_im_tg_ensure_start_retry_timer(), TAG,
                        "Failed to create Telegram startup retry timer");
    if (!esp_timer_is_active(s_tg.start_retry_timer)) {
        err = esp_timer_start_once(s_tg.start_retry_timer,
                                   CAP_IM_TG_START_RETRY_DELAY_MS * 1000ULL);
        ESP_RETURN_ON_ERROR(err, TAG, "Failed to arm Telegram startup retry timer");
    }
