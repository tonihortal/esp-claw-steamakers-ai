...
#define FREE_TIER_SAFE_MAX_TOKENS 1024
#define RECENT_MESSAGES_MAX_BYTES 2048
...
static bool is_groq_endpoint(const openai_compatible_backend_ctx_t *ctx)
{
    return ctx && ctx->base_url && strstr(ctx->base_url, "api.groq.com") != NULL;
}

static bool is_cerebras_endpoint(const openai_compatible_backend_ctx_t *ctx)
{
    return ctx && ctx->base_url && strstr(ctx->base_url, "api.cerebras.ai") != NULL;
}

static bool is_mistral_endpoint(const openai_compatible_backend_ctx_t *ctx)
{
    return ctx && ctx->base_url && strstr(ctx->base_url, "api.mistral.ai") != NULL;
}

static bool is_google_ai_endpoint(const openai_compatible_backend_ctx_t *ctx)
{
    return ctx && ctx->base_url &&
           strstr(ctx->base_url, "generativelanguage.googleapis.com") != NULL;
}

static bool needs_portable_message_history(const openai_compatible_backend_ctx_t *ctx)
{
    return is_groq_endpoint(ctx) || is_cerebras_endpoint(ctx) ||
           is_mistral_endpoint(ctx) || is_google_ai_endpoint(ctx);
}

static void sanitize_message_for_endpoint(const openai_compatible_backend_ctx_t *ctx,
                                          cJSON *message)
{
    ...
    /*
     * Some OpenAI-compatible providers reject metadata persisted from another
     * provider, or even metadata from their own response when it is replayed.
     * Tool calls remain intact; only non-portable fields are removed.
     */
    if (needs_portable_message_history(ctx)) {
        ...
        cJSON_DeleteItemFromObjectCaseSensitive(message, "is_error");
        cJSON_DeleteItemFromObjectCaseSensitive(message, "refusal");
    }

    /* Gemini's OpenAI compatibility layer rejects an assistant tool-call
     * message when content is JSON null, including messages originally
     * produced by another provider and replayed from session history. Keep a
     * non-empty string alongside tool_calls so the turn can be replayed. */
    if (is_google_ai_endpoint(ctx)) {
        cJSON *content = cJSON_GetObjectItemCaseSensitive(message, "content");
        cJSON *tool_calls = cJSON_GetObjectItemCaseSensitive(message, "tool_calls");

        if (cJSON_IsNull(content) && cJSON_IsArray(tool_calls) &&
                cJSON_GetArraySize(tool_calls) > 0) {
            cJSON_ReplaceItemInObjectCaseSensitive(message, "content",
                                                   cJSON_CreateString("[tool call]"));
        }
    }
}

static bool message_has_role(const cJSON *message, const char *expected_role)
{
    const cJSON *role = NULL;

    if (!message || !cJSON_IsObject((cJSON *)message) || !expected_role) {
        return false;
    }
    role = cJSON_GetObjectItemCaseSensitive((cJSON *)message, "role");
    return cJSON_IsString((cJSON *)role) && role->valuestring &&
           strcmp(role->valuestring, expected_role) == 0;
}

static int recent_messages_start(const cJSON *source)
{
    int count = cJSON_GetArraySize((cJSON *)source);
    int selected_start = -1;
    size_t selected_bytes = 0;

    /* Select complete recent turns. A turn begins with a user message, so the
     * returned suffix never starts with an orphaned tool result. The newest
     * turn is retained even when its user content alone exceeds the budget. */
    for (int i = count - 1; i >= 0; i--) {
        cJSON *item = cJSON_GetArrayItem((cJSON *)source, i);
        char *encoded = cJSON_PrintUnformatted(item);
        size_t item_bytes = encoded ? strlen(encoded) : 0;

        cJSON_free(encoded);
        if (selected_start >= 0 &&
                selected_bytes + item_bytes > RECENT_MESSAGES_MAX_BYTES) {
            break;
        }
        selected_bytes += item_bytes;
        if (message_has_role(item, "user")) {
            selected_start = i;
        }
    }

    return selected_start >= 0 ? selected_start : 0;
}
...
static esp_err_t parse_chat_response(const openai_compatible_backend_ctx_t *ctx,
                                     const char *body,
                                     ...
                                     char **out_error_message)
{
    ...
    /* Store only fields that this endpoint will accept again on the next
     * multi-turn request. */
    sanitize_message_for_endpoint(ctx, message);
                                 ...
                                 char **out_error_message)
{
    ...
    char *post_data = NULL;
    uint32_t request_max_tokens = ctx->max_tokens;
    ...
    if (needs_portable_message_history(ctx)) {
        if (request_max_tokens > FREE_TIER_SAFE_MAX_TOKENS) {
            request_max_tokens = FREE_TIER_SAFE_MAX_TOKENS;
        }
    }
    if (is_groq_endpoint(ctx)) {
        cJSON_AddFalseToObject(body, "include_reasoning");
    }
    cJSON_AddNumberToObject(body, profile->max_tokens_field, request_max_tokens);
    ...
    if (needs_portable_message_history(ctx)) {
        first_message_index = recent_messages_start(request->messages);
    }

    cJSON_ArrayForEach(item, request->messages) {
        cJSON *dup = NULL;

        if (message_index++ < first_message_index) {
            continue;
        }
        ...
        sanitize_message_for_endpoint(ctx, dup);
                                        ...
                                        char **out_error_message)
{
    ...
    err = parse_chat_response(ctx, http_response.body, out_response, out_error_message);
                                               ...
                                               char **out_error_message)
{
    ...
    err = parse_chat_response(ctx, http_response.body, &response, out_error_message);
    if (err != ESP_OK) {
        goto cleanup;
    }
