...
static int lua_display_init(lua_State *L)
{
    ...
    esp_err_t err;
    if (direct) {
        if (panel_if != DISPLAY_HAL_PANEL_IF_IO || io_handle == NULL) {
            return luaL_error(L, "display direct mode requires a panel IO handle");
        }
        if (display_service_is_started()) {
            return luaL_error(L, "display direct mode unavailable while display service is active");
        }
        s_display_session = NULL;
    } else {
