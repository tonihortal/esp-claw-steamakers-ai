                             ...
                             int lcd_height)
{
    ...
    ESP_GOTO_ON_FALSE(session == NULL || display_service_session_is_valid(session),
                      ESP_ERR_INVALID_ARG, fail, TAG, "invalid display session");
    ...
    if (display_hal_pixels_need_swap(s_state.pixel_format, s_state.panel_if)) {
        ...
        if (s_state.submit_swap_buffer == NULL) {
            /* Boards without usable PSRAM can still use the direct drawing API.
             * A single scanline is enough because raw blits are synchronous. */
            swap_bytes = (size_t)lcd_width * bytes_per_pixel;
            s_state.submit_swap_buffer = heap_caps_aligned_alloc(
                16, swap_bytes, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        }
        ESP_GOTO_ON_FALSE(s_state.submit_swap_buffer != NULL, ESP_ERR_NO_MEM, fail, TAG,
                          "alloc submit swap buffer failed");
        s_state.submit_swap_buffer_pixels = swap_bytes / bytes_per_pixel;
    }
...
/* Submit through the display service when it owns the panel. In explicit
   direct mode there is no LVGL service; tx_param(-1) drains queued SPI color
   transfers so the caller can safely reuse the scanline buffer on return. */
static esp_err_t display_hal_raw_blit_locked(int x_start, int y_start,
                                             int x_end, int y_end,
                                             const void *pixels)
{
    if (s_state.session != NULL) {
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_draw_bitmap(s_state.panel, x_start, y_start,
                                                   x_end, y_end, pixels),
                        TAG, "direct panel blit failed");
    if (s_state.io != NULL) {
        return esp_lcd_panel_io_tx_param(s_state.io, -1, NULL, 0);
    }
    return ESP_OK;
}
                                                  ...
                                                  int pending_framebuffer_index)
{
    ...
    if (display_hal_pixels_need_swap(s_state.pixel_format, s_state.panel_if)) {
        size_t row_pixels = (size_t)(x_end - x_start);
        size_t row_count = (size_t)(y_end - y_start);
        size_t pixel_count = row_pixels * row_count;
        ESP_RETURN_ON_FALSE(s_state.submit_swap_buffer != NULL, ESP_ERR_INVALID_STATE, TAG, "submit swap buffer missing");
        if (pixel_count > s_state.submit_swap_buffer_pixels) {
            ESP_RETURN_ON_FALSE(row_pixels <= s_state.submit_swap_buffer_pixels,
                                ESP_ERR_NO_MEM, TAG, "submit swap buffer too small");
            for (size_t row = 0; row < row_count; ++row) {
                const uint8_t *src = (const uint8_t *)pixels +
                                     row * row_pixels * s_state.bytes_per_pixel;
                display_hal_bswap16_into(s_state.submit_swap_buffer, src, row_pixels);
                ESP_RETURN_ON_ERROR(
                    display_hal_raw_blit_locked(x_start, y_start + (int)row,
                                                x_end, y_start + (int)row + 1,
                                                s_state.submit_swap_buffer),
                    TAG, "scanline blit failed");
            }
    ...
    ret = display_hal_raw_blit_locked(x_start, y_start, x_end, y_end, submit_pixels);
    if (ret != ESP_OK) {
        return ret;
    }
                                                    ...
                                                    const uint8_t *fb)
{
    ...
    if (s_state.submit_swap_buffer != NULL) {
        ...
        if (total_bytes <= buffer_bytes) {
            ...
            return display_hal_raw_blit_locked(x, y, x + w, y + h, dst);
        }
    }
