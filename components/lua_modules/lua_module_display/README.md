...
### `display.init(panel_handle, io_handle, lcd_width, lcd_height[, panel_if[, pixel_format[, direct]]])`
...
- `direct`: optional boolean, default `false`. When `true`, bypasses the LVGL
  display service and submits synchronous raw panel transfers. Use it only on
  constrained boards where the display service is not running; frame-buffered
  APIs still require enough PSRAM, while opaque direct drawing uses one scanline.
