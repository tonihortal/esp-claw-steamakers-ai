...
static void lua_module_storage_push_size(lua_State *L, size_t value)
{
    if (value <= (size_t)LUA_MAXINTEGER) {
        lua_pushinteger(L, (lua_Integer)value);
    } else {
        /* Some embedded Lua builds use 32-bit integers. lua_Number retains
         * exact byte counts throughout the practical FATFS size range. */
...
static int lua_module_storage_get_free_space(lua_State *L)
{
    ...
    lua_newtable(L);
    lua_module_storage_push_size(L, total);
    lua_setfield(L, -2, "total");
    lua_module_storage_push_size(L, free_bytes);
    lua_setfield(L, -2, "free");
    lua_module_storage_push_size(L, total - free_bytes);
