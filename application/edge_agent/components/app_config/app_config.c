...
void app_config_to_claw(const app_config_t *config, app_claw_config_t *out)
{
    ...
    /* An empty value normally exposes every default tool group. That prompt is
     * too large for Groq's free TPM budget, so this board starts with the two
     * routing groups it needs. Activating a skill can still expose any other
     * enabled capability group for the current session. */
    strlcpy(out->llm_visible_cap_groups,
            config->llm_visible_cap_groups[0] ? config->llm_visible_cap_groups : "cap_lua,cap_skill",
